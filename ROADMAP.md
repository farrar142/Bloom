...
- [x] DB 모듈 연동 (`Session`, `AsyncSession` → `AutoCloseable`)
...
- [ ] `EnvStr["API_KEY"]` - 설정값 주입, str,int,float~~~
- [ ] `@ConfigurationProperties` - 설정 클래스, yml, .env 로더
- [ ] 환경별 설정 (dev, prod)

### 4.2 보안

- [x] 인증 미들웨어
- [x] `@Authenticated` 데코레이터
- [x] JWT 지원
...
1. [x] `@ExceptionHandler` - 예외 핸들러
2. [x] 전역 에러 핸들링 미들웨어
3. [x] HTTP 상태 코드 매핑
4. [x] `UploadedFile` - 파일 업로드
5. [x] `StreamingResponse`, `SSEResponse`
