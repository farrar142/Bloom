...
`MethodProxy`는 원본 메서드를 감싸서 Advice 체인을 실행하는 래퍼입니다.

```python
class MethodProxy:
    """메서드 프록시 - Advice 체인을 통해 호출"""

    def __init__(
        self,
        container: HandlerContainer,  # 메서드의 컨테이너
        instance: Any,                # 메서드가 속한 인스턴스
        original: Callable,           # 원본 메서드
        manager: MethodInvocationManager,  # Advice 실행 매니저
    ): ...

    def __call__(self, *args, **kwargs) -> Any:
        # MethodInvocationManager.invoke()를 통해 Advice 체인 실행
...
`Application.ready()` 호출 시 다음 순서로 프록시가 적용됩니다:

```
Application.ready()
    └── _apply_method_proxies()
            │
            ├── MethodInvocationManager 생성
            ├── ContainerManager에서 MethodAdviceRegistry 조회
            │
            └── 모든 인스턴스 순회
                    │
                    ├── 일반 메서드 → MethodProxy로 래핑
                    │       setattr(instance, name, proxy)
                    │
                    └── ProxyableDescriptor → apply_proxy() 호출
                            bound_obj = descriptor.apply_proxy(instance, proxy)
                            setattr(instance, name, bound_obj)
```

### ProxyableDescriptor 추상 클래스

디스크립터(`@Task` 등)가 프록시 적용을 지원하기 위한 추상 베이스 클래스입니다.
...
### TaskDescriptor의 ProxyableDescriptor 구현

`@Task` 데코레이터의 `TaskDescriptor`가 이 ABC를 상속합니다:

```python
from bloom.core.abstract import ProxyableDescriptor

class TaskDescriptor(ProxyableDescriptor, Generic[T]):
    """@Task 데코레이터가 반환하는 디스크립터"""

    def __init__(self, handler: Callable, element: TaskElement):
    ...
    def __get__(self, instance, owner) -> TaskDescriptor | BoundTask:
        if instance is None:
            return self
        return BoundTask(self._handler, instance, element=self._element)
```

### BoundTask의 프록시 사용

`BoundTask.__call__`은 프록시가 설정되어 있으면 프록시를 통해 호출합니다:
...
class BoundTask(Generic[T]):
    ...
    def __call__(self, *args, **kwargs) -> T:
        ...
        if self._use_proxy and self._proxy is not None:
            # MethodProxy → MethodInvocationManager → Advice 체인
...
새로운 디스크립터를 만들 때 `ProxyableDescriptor`를 상속하면
자동으로 프록시가 적용됩니다:

```python
from bloom.core.abstract import ProxyableDescriptor

class CachedDescriptor(ProxyableDescriptor):
    """@Cached 데코레이터용 디스크립터"""

    def __init__(self, handler: Callable, ttl: int):
        self._handler = handler
        self._ttl = ttl

    # ProxyableDescriptor 구현 (필수!)
    def get_original_handler(self) -> Callable:
        return self._handler

    def apply_proxy(self, instance: Any, proxy: Any) -> "BoundCached":
        bound = BoundCached(self._handler, instance, self._ttl)
        bound._proxy = proxy
        return bound
    ...
    def __get__(self, instance, owner):
        if instance is None:
            return self
        return BoundCached(self._handler, instance, self._ttl)
...
def Cached(ttl: int = 60):
    ...
    def decorator(method):
        container = HandlerContainer.get_or_create(method)
        container.add_elements(CacheableElement(ttl))
        return CachedDescriptor(method, ttl)
    return decorator
```
...
bloom/core/
├── abstract/
│   └── proxyable.py     # ProxyableDescriptor ABC
├── advice/
...
bloom/task/
└── decorator.py         # TaskDescriptor, BoundTask (ProxyableDescriptor 상속)
```
