...
class TestCondition:
    ...
    def test_basic_condition_to_sql(self):
        """기본 조건 SQL 생성"""
...
class TestConditionGroup:
    ...
    def test_and_group_to_sql(self):
        """AND 그룹 SQL 생성"""
        cond1 = Condition("name", "=", "alice")
        cond2 = Condition("age", ">", 18)
    ...
    def test_group_chaining_and(self):
        """그룹 체이닝 - AND"""
        cond1 = Condition("a", "=", 1)
        cond2 = Condition("b", "=", 2)
        cond3 = Condition("c", "=", 3)
    ...
    def test_group_chaining_or(self):
        """그룹 체이닝 - OR"""
        cond1 = Condition("a", "=", 1)
        cond2 = Condition("b", "=", 2)
        cond3 = Condition("c", "=", 3)
...
class TestFieldExpression:
    ...
    def test_eq_none(self, field_expr):
        """== None은 IS NULL"""
        cond = field_expr == None
        assert cond.operator == "IS"

    def test_ne(self, field_expr):
        """!= 연산"""
        cond = field_expr != "bob"
        assert cond.operator == "!="

    def test_ne_none(self, field_expr):
        """!= None은 IS NOT NULL"""
        cond = field_expr != None
        assert cond.operator == "IS NOT"

    def test_gt(self, age_expr):
        """> 연산"""
        cond = age_expr > 18
        assert cond.operator == ">"
        assert cond.value == 18

    def test_ge(self, age_expr):
        """>= 연산"""
        cond = age_expr >= 18
        assert cond.operator == ">="

    def test_lt(self, age_expr):
        """< 연산"""
        cond = age_expr < 65
        assert cond.operator == "<"

    def test_le(self, age_expr):
        """<= 연산"""
        cond = age_expr <= 65
        assert cond.operator == "<="
    ...
    def test_is_null(self, field_expr):
        """IS NULL"""
        cond = field_expr.is_null()
        assert cond.operator == "IS"
        assert cond.value is None

    def test_is_not_null(self, field_expr):
        """IS NOT NULL"""
    ...
    def test_repr(self, field_expr):
        """repr"""
        assert "name" in repr(field_expr)

    def test_hash(self, field_expr):
        """hash"""
...
class TestExpressionsIntegration:
    ...
    def test_complex_condition_sql(self):
        """복잡한 조건 SQL 생성"""
