...
class TestEmptyModule:
    ...
    def test_scan_empty_module(self, reset_container_manager):
        """컴포넌트가 없는 모듈 스캔"""
        import types
        ...
        app.scan(empty_module).ready()

        # 에러 없이 정상 동작
        assert app.manager is not None

    def test_scan_module_with_only_functions(self, reset_container_manager):
        """함수만 있는 모듈 스캔"""
        import types
...
class TestFactoryEdgeCases:
    ...
    def test_factory_returns_none(self, reset_container_manager):
        ...
        app = Application("none_factory").scan(NoneFactory, Consumer).ready()
        consumer = app.manager.get_instance(Consumer)

        assert consumer.value.data is None

    def test_factory_returns_empty_list(self, reset_container_manager):
        """@Factory가 빈 리스트 반환"""
        ...
        app = Application("empty_list").scan(EmptyListFactory).ready()
        result = app.manager.get_instance(list[str])

        assert result == []

    def test_factory_with_no_args(self, reset_container_manager):
        """@Factory 메서드에 인자 없음"""
        ...
        app = Application("no_arg").scan(NoArgFactory).ready()
...
class TestComponentEdgeCases:
    ...
    def test_component_with_class_variable(self, reset_container_manager):
        """클래스 변수가 있는 컴포넌트"""
        ...
        app = Application("class_var").scan(WithClassVar).ready()
        instance = app.manager.get_instance(WithClassVar)

        assert instance.class_var == "class_value"
        assert instance.increment() == 1

    def test_component_with_property(self, reset_container_manager):
        """@property가 있는 컴포넌트"""
        ...
        app = Application("property").scan(WithProperty).ready()
        instance = app.manager.get_instance(WithProperty)

        assert instance.value == 20

    def test_component_inheritance(self, reset_container_manager):
        """상속 관계의 컴포넌트"""
        ...
        app = Application("inheritance").scan(DerivedService).ready()
...
class TestDependencyEdgeCases:
    ...
    def test_optional_dependency_missing(self, reset_container_manager):
        """Optional 의존성이 없는 경우"""
        ...
        app = Application("optional_missing").scan(OptionalConsumer).ready()
        instance = app.manager.get_instance(OptionalConsumer)

        assert instance.service is None

    def test_multiple_implementations_list(self, reset_container_manager):
        ...
        app = Application("multi_impl").scan(ServiceA, ServiceB).ready()
    ...
    def test_lazy_dependency(self, reset_container_manager):
        """Lazy 의존성 기본 동작"""
        ...
        app = Application("lazy").scan(HeavyService, Consumer).ready()
        consumer = app.manager.get_instance(Consumer)
...
class TestLifecycleEdgeCases:
    ...
    def test_postconstruct_exception(self, reset_container_manager):
    ...
    def test_multiple_postconstruct(self, reset_container_manager):
...
class TestScopeEdgeCases:
    ...
    def test_singleton_is_same_instance(self, reset_container_manager):
        ...
        app = Application("singleton").scan(SingletonService).ready()
    ...
    def test_prototype_is_different_instance(self, reset_container_manager):
        ...
        app = Application("prototype").scan(PrototypeService, Consumer).ready()
...
class TestTypeAnnotationEdgeCases:
    ...
    def test_generic_type_injection(self, reset_container_manager):
        """제네릭 타입 주입"""
        from dataclasses import dataclass
        ...
        app = Application("generic").scan(ConfigFactory, Consumer).ready()
        consumer = app.manager.get_instance(Consumer)

        assert consumer.config.value == "test_value"

    def test_optional_with_default(self, reset_container_manager):
        """Optional 타입과 기본값"""
        ...
        app = Application("optional_default").scan(Consumer).ready()
        consumer = app.manager.get_instance(Consumer)
