...
class TestPostConstructEdgeCases:
    ...
    async def test_postconstruct_exception_stops_initialization(
    ...
    async def test_multiple_postconstruct_all_called(self, reset_container_manager):
    ...
    async def test_postconstruct_sync(self, reset_container_manager):
        """동기 @PostConstruct"""
        ...
        app = await Application("sync_init").scan(SyncInit).ready_async()
        app.manager.get_instance(SyncInit)

        assert "sync_init" in initialized

    async def test_postconstruct_with_dependency(self, reset_container_manager):
        ...
        app = await Application("init_dep").scan(Config, ServiceWithInit).ready_async()
...
class TestPreDestroyEdgeCases:
    ...
    async def test_predestroy_called_on_shutdown(self, reset_container_manager):
    ...
    async def test_multiple_predestroy_all_called(self, reset_container_manager):
    ...
    async def test_predestroy_reverse_order(self, reset_container_manager):
...
class TestPrototypeLifecycle:
    ...
    async def test_prototype_via_field_injection(self, reset_container_manager):
...
class TestFactoryLifecycle:
    ...
    async def test_factory_created_instance_postconstruct(
        self, reset_container_manager
    ):
        ...
        app = await Application("factory_lifecycle").scan(Config).ready_async()
        service = app.manager.get_instance(ExternalService)

        assert service.initialized is True

    async def test_factory_created_instance_predestroy(self, reset_container_manager):
...
class TestLifecycleWithExceptions:
    ...
    async def test_partial_init_cleanup(self, reset_container_manager):
        """부분 초기화 후 실패 시 정리"""
    ...
    async def test_lifecycle_with_none_return(self, reset_container_manager):
        ...
        app = await Application("return_value").scan(ReturnValue).ready_async()
