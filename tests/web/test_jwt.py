...
class TestJwtEncoder:
    ...
    async def test_encode_basic(self, encoder: JwtEncoder):
        """기본 인코딩"""
    ...
    async def test_encode_with_payload(self, encoder: JwtEncoder):
        """payload 데이터 인코딩"""
        payload = {"sub": "user123", "role": "admin", "name": "John"}
        token = encoder.encode(payload)
        assert isinstance(token, str)

    async def test_encode_adds_iat(self, encoder: JwtEncoder, decoder: JwtDecoder):
        """iat (issued at) 자동 추가"""
        token = encoder.encode({"sub": "user123"})
    ...
    async def test_encode_adds_exp(self, encoder: JwtEncoder, decoder: JwtDecoder):
    ...
    async def test_encode_with_custom_expires_in(
    ...
    async def test_encode_adds_issuer(self, encoder: JwtEncoder, decoder: JwtDecoder):
        """iss (issuer) 자동 추가"""
        token = encoder.encode({"sub": "user123"})
        decoded = decoder.decode(token)
        assert decoded["iss"] == "test-issuer"

    async def test_encode_adds_audience(self, encoder: JwtEncoder, decoder: JwtDecoder):
        """aud (audience) 자동 추가"""
        token = encoder.encode({"sub": "user123"})
        decoded = decoder.decode(token)
        assert decoded["aud"] == "test-audience"

    async def test_encode_extra_claims(self, encoder: JwtEncoder, decoder: JwtDecoder):
        """추가 클레임"""
        token = encoder.encode({"sub": "user123"}, custom_claim="value")
        decoded = decoder.decode(token)
        assert decoded["custom_claim"] == "value"

    async def test_encode_no_expiration(self, config: MyTestJwtConfig):
        """만료 없는 토큰"""
        config.expires_in = 0
...
class TestJwtDecoder:
    ...
    async def test_decode_basic(self, encoder: JwtEncoder, decoder: JwtDecoder):
        """기본 디코딩"""
    ...
    async def test_decode_invalid_token(self, decoder: JwtDecoder):
        """잘못된 토큰"""
        with pytest.raises(JwtInvalidTokenError):
            decoder.decode("invalid.token.here")

    async def test_decode_empty_token(self, decoder: JwtDecoder):
        """빈 토큰"""
        with pytest.raises(JwtInvalidTokenError):
            decoder.decode("")

    async def test_decode_wrong_signature(
        self, encoder: JwtEncoder, config: MyTestJwtConfig
    ):
        """서명 불일치"""
        token = encoder.encode({"sub": "user123"})
    ...
    async def test_decode_expired_token(self, config: MyTestJwtConfig):
        """만료된 토큰"""
        import jwt as pyjwt
    ...
    async def test_decode_unverified(
        self, encoder: JwtEncoder, config: MyTestJwtConfig
    ):
        """검증 없이 디코딩"""
        token = encoder.encode({"sub": "user123"})
    ...
    async def test_get_unverified_header(
        self, encoder: JwtEncoder, decoder: JwtDecoder
    ):
        """헤더 추출"""
        token = encoder.encode({"sub": "user123"})
...
class TestJwtCodec:
    ...
    async def test_encode_decode(self, codec: JwtCodec):
        """인코딩 + 디코딩"""
    ...
    async def test_codec_has_encoder_decoder(self, codec: JwtCodec):
        """encoder/decoder 속성"""
        assert isinstance(codec.encoder, JwtEncoder)
        assert isinstance(codec.decoder, JwtDecoder)

    async def test_decode_unverified(self, codec: JwtCodec):
        """검증 없이 디코딩"""
        token = codec.encode({"sub": "user123"})
        decoded = codec.decode_unverified(token)
        assert decoded["sub"] == "user123"

    async def test_get_unverified_header(self, codec: JwtCodec):
        """헤더 추출"""
        token = codec.encode({"sub": "user123"})
...
class TestJwtConfig:
    ...
    async def test_custom_algorithm(self):
        """커스텀 알고리즘"""
    ...
    async def test_leeway(self, config: MyTestJwtConfig):
        """leeway (시간 허용 오차)"""
        import jwt as pyjwt
        from datetime import datetime, timezone
...
class TestJwtErrors:
    ...
    async def test_asymmetric_needs_private_key(self):
    ...
    async def test_asymmetric_decode_needs_public_key(self):
...
class TestSerializableJwt:
    ...
    async def test_encode_object(self, encoder: JwtEncoder, decoder: JwtDecoder):
        """Serializable 객체 인코딩"""
        payload = UserPayload(
    ...
    async def test_decode_as(self, encoder: JwtEncoder, decoder: JwtDecoder):
        """타입 안전한 디코딩"""
    ...
    async def test_codec_encode_object(self, codec: JwtCodec):
    ...
    async def test_codec_decode_as(self, codec: JwtCodec):
    ...
    async def test_serializable_protocol(self):
        """Serializable 프로토콜 확인"""
        payload = UserPayload(user_id="test", role="test")
        assert isinstance(payload, Serializable)

    async def test_jwt_claims_excluded(self, encoder: JwtEncoder, decoder: JwtDecoder):
    ...
    async def test_round_trip(self, codec: JwtCodec):
