...
class TestEventSystemIntegration:
    ...
    async def test_full_event_lifecycle(self, event_bus, registrar):
        ...
        assert (
            len(results) >= 3
        )  # OrderService의 order.created 핸들러 3개 + AuditService 1개

        # 핸들러 실행 결과 확인
        assert "ORD-001" in order_service.processed_orders
        assert any(
            "notification:ORD-001" in n for n in order_service.notifications_sent
        )
    ...
    async def test_error_isolation_between_handlers(self, event_bus, registrar):
        ...
        success_results = [
            r for r in event_results if r.status == EventStatus.COMPLETED
        ]
...
class TestMultiServiceEventIntegration:
    ...
    async def test_cross_service_event_propagation(self, event_bus, registrar):
        ...
        # AuditService 로그 확인
        user_logs = [
            log for log in audit_service.logs if log.get("action") == "user_created"
        ]
...
class TestDomainEventIntegration:
    ...
    async def test_event_correlation_and_causation(self, event_bus):
        ...
        notification_event = next(
            e for e in events if e.event_type == "notification.scheduled"
        )
...
class TestEventSubscriptionManagement:
    ...
    async def test_subscription_group_priority_ordering(self, event_bus):
        ...
        await event_bus.subscribe(
            "test.order", handler_10, priority=10, mode=SubscriptionMode.SYNC
        )
        await event_bus.subscribe(
            "test.order", handler_100, priority=100, mode=SubscriptionMode.SYNC
        )
        await event_bus.subscribe(
            "test.order", handler_50, priority=50, mode=SubscriptionMode.SYNC
        )
...
class TestEventResultTracking:
    ...
    async def test_event_result_aggregation(self, event_bus):
        ...
        await event_bus.subscribe(
            "result.test", handler_success, mode=SubscriptionMode.SYNC
        )
        await event_bus.subscribe(
            "result.test", handler_error, mode=SubscriptionMode.SYNC
        )
        await event_bus.subscribe(
            "result.test", handler_slow, mode=SubscriptionMode.SYNC
        )
...
class TestRealWorldScenarios:
    ...
    @pytest.mark.asyncio
    async def test_user_registration_with_multiple_side_effects(
        ...
        class RewardService:
            @AsyncEventListener(
                "user.registered", condition="payload.get('referral_code') is not None"
            )
            async def credit_referral_bonus(self, event: Event):
                side_effects.append(
                    f"referral_credited:{event.payload['referral_code']}"
                )
