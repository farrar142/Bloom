...
class TestApplication:
    ...
    async def test_application_sets_name(self):
        """Application이 app_name을 설정"""
        app = Application("my_app")
        assert app.manager.app_name == "my_app"

    async def test_scan_and_ready(self):
        """새 API: scan().ready() 체이닝"""
        app = await Application("test_app").scan(conftest).ready_async()
    ...
    async def test_router_initialized_on_ready(self):
        """ready() 호출 시 라우터 초기화"""
        app = await Application("test_app").scan(conftest).ready_async()
    ...
    async def test_asgi_property(self):
        """ASGI 애플리케이션 속성"""
        app = await Application("test_app").scan(conftest).ready_async()
