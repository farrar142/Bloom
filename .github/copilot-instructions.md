...
bloom은 Spring-style 의존성 주입 컨테이너로, 데코레이터 기반으로 컴포넌트를 등록하고 자동 의존성 해결을 제공합니다.

### Core Components

- **Application** (`bloom/application.py`): 최상위 진입점. `scan().ready()` 체이닝으로 초기화
- **Container** (`bloom/core/container/`): 클래스 메타데이터와 의존성 정보 관리
- **ContainerManager** (`bloom/core/manager.py`): 전역 레지스트리 (컨테이너/인스턴스)
- **Web Layer** (`bloom/web/`): ASGI, Router, Controller, HttpMethodHandler
...
```python
from bloom import Application, Component, Controller, Get
...
| 파일                       | 역할                                         |
| -------------------------- | -------------------------------------------- |
| `bloom/application.py`     | 최상위 Application 클래스                    |
| `bloom/__init__.py`        | 주요 API export                              |
| `bloom/core/decorators.py` | `@Component`, `@Factory`, `@Handler`         |
| `bloom/core/manager.py`    | 전역 상태 관리, 인스턴스 조회                |
| `bloom/core/utils.py`      | 토폴로지컬 정렬 (순환 의존성 감지)           |
| `bloom/core/container/`    | Container 구현체들                           |
| `bloom/web/asgi.py`        | ASGI 애플리케이션                            |
| `bloom/web/router.py`      | HTTP 라우팅                                  |
| `bloom/web/controller.py`  | `@Controller`, `@RequestMapping`             |
| `bloom/web/handler.py`     | `@Get`, `@Post`, `@Put`, `@Patch`, `@Delete` |
| `bloom/web/http.py`        | `HttpRequest`, `HttpResponse`                |
